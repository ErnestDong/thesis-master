...
model1 = smf.ols(
    "log(Premium) ~ Neighbor*Post+Prem_before+log(Price)+log(Penetration)+log(GDP)",
...
model21 = smf.ols(
    "log(Coverage) ~ Neighbor*Post+Prem_before+log(Price)+log(Penetration)+log(GDP)",
    data=near[(near["Disaster"] == 0)],
).fit()

model22 = smf.ols(
    "log(Coverage) ~ Disaster*Post+Prem_before+log(Price)+log(Penetration)+log(GDP)",
    data=near[(near["Neighbor"] == 0)],
).fit()
# %%

model2 = smf.ols(
    "log(Premium) ~ Disaster*Post+Prem_before+log(Price)+log(Penetration)+log(GDP)",
...
original_df = pd.read_parquet("../data/ols_ups_far.parquet")
